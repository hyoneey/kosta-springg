...
public class ReplyController {
	...
	//페이징 처리- ajax로 호출 될 것이므로 model객체를 사용할 수 없어, 전달하는 데이터를 담기위해 map객체를 별도 생성
	...
	public ResponseEntity<Map<String, Object>> listPage( @PathVariable("bno") Integer bno, @PathVariable("page") Integer page){
		...
		try {
			...
			Map<String, Object> map = new HashMap<String, Object>(); //  페이징 처리된 댓글의 목록(list)와  pageMaker 클래스의 객체를 담는다.
			List<ReplyVO> list = service.listReplyPage(bno, cri); //댓글 페이징 처리 리스트
